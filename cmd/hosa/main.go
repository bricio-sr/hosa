...
func main() {
	...
	// --- Configuration: file → CLI flags (CLI always wins) ---
	configPath := flag.String("config", config.DefaultConfigPath, "path to TOML config file")

	// Pre-parse only --config before loading the file, so other flags
	// can use file values as their defaults.
	flag.Parse()

	cfg, err := config.Load(*configPath)
	if err != nil {
		log.Fatalf("HOSA [FATAL] config_load_failed path=%s err=%v", *configPath, err)
	}

	// Re-register flags with file values as defaults, then parse again.
	// This implements: file values → CLI flag defaults → actual CLI override.
